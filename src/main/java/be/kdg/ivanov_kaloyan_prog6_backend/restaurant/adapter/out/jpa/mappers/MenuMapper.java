...
@Mapper(componentModel = "spring", uses = {MoneyMapper.class, DishMapper.class})
public interface MenuMapper {

    @ObjectFactory
    default Menu createMenu(MenuJpaEntity jpa, @Context MoneyMapper moneyMapper, @Context DishMapper dishMapper) {
        RestaurantId restaurantId = RestaurantId.of(jpa.getRestaurantId());
        Map<UUID, Dish> dishes = dishMapper.toDomainMap(jpa.getDishes(), moneyMapper);
    ...
    default Menu toDomain(MenuJpaEntity jpa, @Context MoneyMapper moneyMapper, @Context DishMapper dishMapper) {
        return createMenu(jpa, moneyMapper, dishMapper);
    }
